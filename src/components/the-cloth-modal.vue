...
const saveItem = () => {
  if (!selectedSize.value) return alert('Выберите размер')
        ...
        <section class="flex h-full w-full flex-col items-start justify-between gap-2 md:w-2/5 md:py-4">
          <article class="flex h-full w-full flex-col items-start justify-between space-y-6">
            ...
            <div class="flex flex-col items-start justify-center gap-4">
              ...
              <!-- <p class="text-xs font-bold text-700/70">#Просьба внимательно выбирать товар#</p> -->
            </div>

            <div class="flex flex-col gap-1">
              <h3 class="text-base text-700/70">Характеристики товара</h3>
            ...
            <div class="flex flex-col gap-1">
              <h3 class="text-base text-700/70">Цвет</h3>
            ...
            <div class="flex flex-col items-start justify-center">
              <span v-if="price" class="text-xl font-bold text-700/70">
                {{ price.toLocaleString('pt-RU', {
                  currency: 'RUB',
                  style: 'currency'
                }) }}
              </span>
              <!-- <span v-if="parcel && parcel_price" class="text-base text-700/70">
                {{ parcel }}x  {{
                  parcel_price.toLocaleString('pt-RU', {
                    currency: 'RUB',
                    style: 'currency'
                  }) }} sem juros</span> -->
            </div>
            ...
            <div class="flex w-full flex-col items-start justify-between gap-6">
              ...
              <button class="w-full bg-700 p-4 text-base uppercase text-800" @click="saveItem">Купить</button>
            </div>
          </article>
        </section>
      </article>
